...
export const translation = pgTable(
  ...
  (table) => ({
    ...
    uniqueProjectLocale: unique().on(table.projectId, table.locale),
  }),
);
...
export const newsletterSubscriber = pgTable('newsletter_subscriber', {
  id: text('id')
    ...
    .$defaultFn(() => createId()),
  email: text('email').notNull().unique(),
  locale: text('locale', { enum: ['fr', 'en', 'de', 'it', 'es'] })
    .notNull()
    .default('fr'),
  createdAt: timestamp('createdAt').notNull().defaultNow(),
});
...
export const translationRelations = relations(translation, ({ one }) => ({
  ...
  project: one(project, {
    fields: [translation.projectId],
    references: [project.id],
    relationName: 'projectTranslations',
  }),
}));
...
export const projectRelations = relations(project, ({ one, many }) => ({
  ...
  translations: many(translation, { relationName: 'projectTranslations' }),
