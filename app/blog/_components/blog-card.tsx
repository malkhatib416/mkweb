...
const BlogCard: React.FC<BlogCardProps> = ({
  ...
  if (variant === 'horizontal') {
    return (
      <article
        onClick={handleClick}
        className="group cursor-pointer flex gap-6 items-center p-4 rounded-3xl transition-all duration-300 hover:bg-slate-50 border border-transparent hover:border-slate-100"
      >
        <div className="relative w-32 h-32 md:w-48 md:h-32 shrink-0 overflow-hidden rounded-2xl bg-slate-100">
          {post.image ? (
            <img
              src={post.image}
              alt={post.title}
            ...
            <div className="flex items-center justify-center h-full text-slate-200 font-black">
              MK
            </div>
          )}
        </div>
        <div className="flex-1 space-y-2">
          ...
          <h3 className="text-lg md:text-xl font-bold text-slate-900 line-clamp-2 leading-tight group-hover:text-myorange-100 transition-colors">
            {post.title}
          </h3>
          <p className="hidden md:block text-sm text-slate-500 line-clamp-2">
            {post.description}
          </p>
        </div>
      </article>
  ...
  return (
    ...
    >
      <div className="flex flex-col h-full bg-white rounded-[2rem] overflow-hidden border border-slate-100 shadow-sm transition-all duration-300 hover:shadow-xl hover:-translate-y-1 hover:border-myorange-100/10">
        {/* Cover Image */}
        <div className="relative aspect-[3/2] w-full overflow-hidden bg-slate-100">
          {post.image ? (
            ...
            <div className="flex items-center justify-center h-full bg-gradient-to-br from-slate-50 to-slate-100">
              <span className="text-slate-200 text-6xl font-black opacity-10 select-none">
                MK
              </span>
            </div>
          )}
                ...
                <span
                  ...
                  className={
                    category.color
                      ...
                      : 'px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-white/90 text-slate-800 backdrop-blur-md shadow-sm'
                  }
                >
                  {category.name}
                </span>
        ...
        <div className="flex flex-col flex-1 p-8">
          ...
          {/* Title */}
          <h2 className="text-xl md:text-2xl font-bold text-slate-900 mb-3 line-clamp-2 leading-tight group-hover:text-myorange-100 transition-colors">
            {post.title}
          </h2>
